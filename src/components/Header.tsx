...
const socialLinks = [
  {
    label: 'Facebook',
    href: 'https://facebook.com/applianceserviceexperts',
    icon: (
  ...
  {
    label: 'Instagram',
    href: 'https://instagram.com/applianceserviceexperts',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
  ...
  {
    label: 'YouTube',
    href: 'https://youtube.com/@applianceserviceexperts',
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
      ...
      <div className="bg-brand-700 text-white py-1.5 hidden sm:block">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex justify-between items-center">
          <p className="text-xs text-blue-200">Mon–Sun 8AM–8PM · Trichy</p>
          <div className="flex items-center gap-3">
            {socialLinks.map((s) => (
              <a
                ...
                aria-label={s.label}
                className="text-blue-200 hover:text-white transition-colors min-h-[28px] flex items-center"
              >
                {s.icon}
              </a>
            ))}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex flex-col leading-tight min-h-[48px] justify-center">
            <span className="text-lg font-bold text-brand-600">Appliance Service</span>
            ...
            <div className="flex items-center gap-4 mt-4 px-4">
              {socialLinks.map((s) => (
                <a
                  ...
                  rel="noopener noreferrer"
                  aria-label={s.label}
                  className="text-gray-500 hover:text-brand-600 transition-colors min-h-[48px] flex items-center"
                >
                  {s.icon}
                </a>
              ))}
