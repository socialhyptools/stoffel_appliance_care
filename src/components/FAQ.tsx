          ...
          {faqs.map((faq, i) => (
            <div key={i}
              className={`rounded-2xl border transition-all duration-300 overflow-hidden
                ${open === i
                  ? 'border-brand-200 shadow-brand bg-brand-50/30'
                  : 'border-gray-100 bg-white hover:border-gray-200 shadow-soft'
                }`}
            >
              <button
              ...
              >
                <span className={`font-semibold pr-4 transition-colors duration-200 ${open === i ? 'text-brand-700' : 'text-gray-900'}`}>
                  {faq.q}
                </span>
                <span className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300
                  ${open === i ? 'bg-brand-600 text-white rotate-180' : 'bg-gray-100 text-gray-500'}`}>
